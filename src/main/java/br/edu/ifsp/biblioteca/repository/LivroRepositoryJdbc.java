...
public class LivroRepositoryJdbc implements ILivroRepository {
    ...
    private void atualizarLivro(Connection conexao, Livro livro) throws  SQLException{
        ...
        String slq = "UPDATE livro SET isbn = ?, titulo = ?, ano_publicacao = ? WHERE id = ?";

        try(PreparedStatement comando = conexao.prepareStatement(slq)){
            comando.setString(1, livro.getTitulo());
            comando.setString(2, livro.getIsbn());
            comando.setObject(3, livro.getAnoPublicacao());
    ...
    private void salvarAutores(Connection conexao, Livro livro) throws  SQLException {
        ...
        for (Autor autor : livro.getAutores()) {
            ...
            String sqlTdlIntermediaria = "INSERT INTO livro_autor" +
                    "(livro_id, autor_id) VALUES(?, ?)";

            try (PreparedStatement comando = conexao.prepareStatement(sqlTdlIntermediaria)) {
